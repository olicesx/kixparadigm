...
| **编排交接门禁** | `kix-orchestration` 插件 | subagent 交接前校验 sprint marker / plan+progress / blocker / QA 完成度；v2 QA 返回侧一致性校验（subagent/end）；v3 producer_closeout 收尾证据链；v4 sleep 空转等待子代理一次性提醒（bash + pwsh 双平台，2026-08-17 WSL2 实测驱动，共 61 组断言） |
...
```bash
npm test                                  # 全插件回归：门禁 199 + 纪律 63 + 编排 61 + 聚焦 66 + 成本 28 + 路由 67 断言
