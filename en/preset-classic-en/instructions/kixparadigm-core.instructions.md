...
1. **Does the test mirror the real path**: stubs/mocks hide bugs. Keep one real end-to-end path for non-trivial logic, don't test only stubs (fossil evidence in memories/ai-agent-practices "verification-chain discipline")
2. **Is the evidence dimension right**: when claiming "verified", did you verify the real risk point or the surface? (testing that a call chain exists ≠ testing the callee's semantics)
...
> **Run standard lint/tests before every commit** (Rust: fmt --check + clippy -D warnings + test; TS: eslint/prettier/typecheck). Project-specific allowlists/grep gates only need a CI glance when touching that kind of code (fossil evidence in memories "verification-chain discipline")
