...
> **分层**：DSH 常驻认知由 preset 的 `agent.cordis.yml` 活跃 persona 提供；本文件与 `../../instructions/kixparadigm-core.instructions.md` 均为按需参考，不自行承诺流程升级。
...
- **决策 gate（v1.2.11 起软约束）**：发布/评论/合并/破坏性操作默认不执行；用户明确指示即已决策，直接执行，不再逐次询问（见「发布与人类决策」）
...
- 发布/评论/合并/破坏性操作**默认不做**。用户明确指示（如「评论到PR」「发布评论」）就是决策；直接执行，不再重复提问。
- `ask_user_question` 只用于真正缺失的决策信息（发到哪个 PR、APPROVE 还是 COMMENT、范围不明）；不用于机械确认用户已经说过的发布指令。
- 硬 DENY 只保留可机械判定且不可逆的破坏操作（force push / main 保护 / 破坏性 SQL / 控制平面写 / 删除远端数据）。
