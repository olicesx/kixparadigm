...
- **v1.2.15 generalization (self-sensing boundary)**: the trigger moved from a kixparadigm fingerprint to self-sensing — a preset root is a directory with both `agent.cordis.yml` + `preset.yml` (depth ≤2 scan); any workspace with ≥2 roots gets guidance, single-preset/plain workspaces zero-overhead; identity sets default to same-named plugins across roots (N ≥ 2 compared at once), with variants clustered by `PLUGIN_IDENTITY_GROUPS` so write-time and CI share one source instead of re-checking an exemption list; other in-root paths (skills/agents — translation relations) get a **parity hint**: no failure asserted, syncing other roots' counterparts is model judgment (undescribed forms sensed via the reminder, once per session); non-preset-root paths are naturally out (a self-sensing corollary — no per-path exemptions); the full kix contract (budgets/counts/phrases/version pair) is self-declared by carrying `scripts/check-dsh-consistency.cjs` — foreign repos get identity guidance only. Rules are debt; heuristic guidance only. **Shell writes get no mechanical extraction** (a command-path extraction channel was implemented, then removed whole under the debt criterion after live E2E exposed a char-class bug — soft guidance + CI backstop)
