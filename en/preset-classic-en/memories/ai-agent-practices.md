...
> `ask_user_question`）在 DeepSeek Harness 中按 classic 档 `DSH-ADAPTATION.md` §1 映射理解；
...
## 用户可见 bug 的闭环取证（2026-09 tgguard 实证）

- **先证明旧实现会红，再接受新测试为回归测试**：临时反向应用修复或替换真实依赖语义；旧实现仍绿，说明测试或 stub 没覆盖故障点。
- **外部约束必须出现在测试模型里**：Telegram/API/权限等语义若只在生产服务拒绝，mock 永远成功会把真实 bug 隐藏；补最小拒绝规则，并同时保留允许和拒绝两类样本。
- **断言真实边界**：发送/序列化/回调类行为从实际 wire payload 或外部调用记录断言，不只断言内部常量和调用链存在；文档、帮助文本、运维脚本随行为变更一起搜漂移。
- **副作用先验收目标与作用域**：解封、删除、清库或外部请求先确认本地对象存在且作用域精确，再执行副作用；缺失目标必须保证零外部调用。
- **取证边界要写出来**：测试通过只覆盖测试链；外部文档只覆盖协议语义；真机、生产数据和未跑的门禁不要被概括成“整体完成”。
