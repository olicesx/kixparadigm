...
2. **vision-bridge** → `~/.dsh/profiles/web/plugins/dsh-vision-bridge/` (junction + `cordis.patch.yml` entry). Models that declare image input submit images natively, without conversion or the bridge's 8MB limit. Conversion applies only to confirmed text-only models; unknown capabilities use the native host path.
