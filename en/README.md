...
3. **Settings checklist** → warns if `zai-vision` / `zai-coding-cn` providers are missing from `settings.yaml` (needed for `subagent_vision` / `subagent_cross`)

Then restart `dsh web` and pick the **kixparadigm-en** mode in the new-session picker.
