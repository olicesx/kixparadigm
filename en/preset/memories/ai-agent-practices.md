...
## run_code 与直呼工具的取舍（候选，2026-08-18 实证）

- **run_code 侧**：扇出+容错（N 个同类调用，单点失败不拖垮整批、失败项汇总重放）、跨工具结果的变换/聚合、下一步依赖上一步结果的值做分支循环——一步整合省回合。
- **直呼侧**：需要留可回放证据链的观察/验证（验证取证保持 native）、≤4 个操作、需要行号引用支撑后续精确编辑。
- **习惯梯度警告**：系统提示的 per-tool 锚定强度会把判断滑向直呼侧（实证：16 连发 edit 被读前检查拦 4 个，两回合补救；run_code 循环本可一回合消化）。判断依据是任务形状，不是工具偏好；交付时抽查自己有没有用错侧。
