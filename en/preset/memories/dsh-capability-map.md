...
## Task shape → mechanism map (2026-08-17, measured on WSL2 E2E)

> Decision cues live where the model already looks (tool descriptions/catalog: bash's run_in_background, subagent's continuable, workflow's explicit-only semantics all come pre-guided). This table only adds **misuse guards** and the **combined shape** — consult before DSH-related work; do not memorize.

| Task shape | Correct mechanism | Discriminator (misuse guard) |
|---|---|---|
| Single long test/build | Background job: bash/pwsh `run_in_background: true` (param description already guides) → `job_output` collect, `job_kill` stop | Returns a **job id** (e.g. bash-1) |
| Many mechanical tests in parallel (no analysis) | **Pure shell pipeline → single bash command**; needs **tool-call composition / structured logic / self-verification** → `run_code` one round-trip (presentation mode:both) | Measured (2026-08-17): for pure stats bash saves tokens (run_code input ≈ 2×), run_code saves round-trips (1 vs 3) and has a higher write-once-correct rate (embedded baseline self-check); **file contents never enter the context** is the shared advantage of both (vs feeding full text via repeated tools.read) |
| Many tests/files in parallel needing independent analysis | **Concurrent subagent observers** (default, arranger-model guided); explicit `workflow` only for heavy orchestration (multi-stage / large fan-out) | workflow's official description says "ONLY when explicitly asked" — models do not spontaneously use it for parallel checks (measured: 3× subagent instead of workflow) |
| Background subagent (continuable) | `subagent run_in_background: true` → `list_agents` / settlement notice | Returns a **durable subagent id** (not a job id) — do NOT query with `job_output` (measured misuse → unknown job) |
| Combined: background job + workflow | ① start the long task with `run_in_background` → ② workflow (foreground-blocking, does not block the job) → ③ `job_output` collect | Fixed order: background → workflow → collect; collection driven by the completion notice, no polling |

### Mechanical hygiene (2026-08-17, measured lessons)

- **Verification/probe scripts write to `/tmp` (or `/tmp/kix-verify/`) and are cleaned up right after — never into the workspace.** Measured: a verification task left `verify-get-equivalence.test.mjs` in the repo root, polluting `git status` (cleaned); temp dirs leak the same way (kix-orchestration tests leaked `/tmp/kix-orch-test-*` × 593 dirs; now auto-cleaned).
- **subagent id ≠ job id**: `subagent run_in_background` returns a durable agent id — manage with `list_agents`/settlement notice; querying it with `job_output` returns `unknown job` (measured twice; both self-healed after the error, switching to waiting for the settlement notice).
...
| Update | kix-guards v5 (2026-08-15, user decision): ask gates moved to in-chat questions | ✅ code/tests verified (164 assertions); ⚠️ live E2E re-check pending after restart + new session | `plugins/kix-guards.js` v5: human-confirmation points (reset --hard/clean -f/branch -D/stash drop/checkout --/restore/ordinary push/GitHub mutation) changed from `{kind:'ask'}` → approval-service dialog to `ctx.userQuestions.ask()` (the service behind `ask_user_question`) asked inside the chat, user answers 「允许执行/拒绝」; approval policy for `danger-full-access` restored to `never` (web cordis.patch.yml, 2026-08-15) — fully automatic, zero dialogs; degrade paths (no userQuestions / no agent / subagent DELEGATED_CALLER / aborted question) → fail-safe deny |
| Update | kix-guards v7→v9 (v8 = v1.2.10 self-audit; v9 = v1.2.11 publish/comment confirmation gates demoted to soft constraints) | ✅ code/tests verified (current 210 assertions incl. incident regression); deployed to both kixparadigm + kixparadigm-en installs (hash-verified, root plugins/ copy synced to heal drift); ⚠️ live E2E re-check pending after restart + new session | `plugins/kix-guards.js` v7 commit-budget triple fix: ① reflog counting switched to %gs subjects, commit-type entries only (reset/merge/pull/checkout/rebase excluded — no longer punishes the recommended history-cleanup workflow; amend counts toward the hard-cap churn metric only, not the budget); ② marker pointing at a sprint with done.md → warn + fallback to the max-numbered sprint dir; if that is also done → staleAll flag surfaced in the deny message; ③ budget chain progress.commit_budget > plan.task_sizing.derived_commit_budget > plan.blast_radius.max_commits > cold-start 3 (cold start now warns), and an explicit commit_budget: 3 is no longer overridden by the "value equals default" check (latent v6 precedence bug); deny message names the budget source and sprint dir, dropping the misleading "recalc per DAG" wording. Repo side: dae opened sprint-10 (hotfix bookkeeping, commit_budget=8) + marker=10 |
| Update | kix-discipline (2026-08-16, pluginization P0): requirement grill / verification gate mechanized | ✅ code/tests verified (current 68 assertions); ⚠️ live E2E pending after restart | `plugins/kix-discipline.js`: `tools/pre-execute` checks the requirement contract (spec) before implementation edits — no spec + first edit → remind (default) / ask / block; `kix_discipline_spec` tool records goal/xy/assumptions/path/acceptance into workspace `kix-discipline/spec.md`; `agent/turn-stopping` green reminder; `/kix-discipline status|report|on|off`. Intensity defaults to remind; ask/block via config |
| Update | kix-orchestration (2026-08-16, P2 gap fill; v8 negative-semantics QA completion): orchestration handoff gate | ✅ code/tests verified (current 69 assertions); ⚠️ live E2E pending after restart | `plugins/kix-orchestration.js`: `tools/pre-execute` on subagent* dispatch carrying `current_sprint` validates workspace `docs/.kixpower-current-sprint` marker, `docs/sprint-N/{plan,progress}.md` existence, no blocker, QA handoff requires completed==total; bounded port of validate-handoff.ps1 core; deep worktree/SHA/manifest checks not ported |
| Update | kix-focus (2026-08-16, P4 3-layer): minimal + progressive disclosure + PTC | ✅ code/tests verified (current **102 assertions**; 2026-08-17 jobs made resident + tiers/goal auto-activate on first use via capability_call; **2026-08-17 live E2E on WSL2 dsh web @33236**: lite platform-conditional shell + orchestration-member current_sprint mechanical injection) | `plugins/kix-focus.js`: Phase 1 `tools.restrict` trims per-turn tool surface from ~85 (~108KB schema, est. ~30.8K tokens) to resident core (RESTRICT_ALLOW lists only global tools ~11; subagent×5 and kix_capability_* are scope-registered, auto-visible — restrict fails on scope-local names so they stay out of allow); MCP/workflow/goal/job_*/cordis_* on demand; `tools/change` retry; measured **-81.6%** (`scripts/quantify-focus.cjs`); Phase 2 `kix_capability_search` (global view `schemas(undefined)`, group metadata without full schemas) + `kix_capability_call` (global existence check `get(name, undefined)` + proxy via `ctx.tools.execute`, full pre-execute→guards→execute→post-execute, gates still fire; agent-carrying calls not model-direct so no UNKNOWN_TOOL; rootCallId propagated). Sensing design (revised): NO pre-execute deny — restrict already hides trimmed tools (direct calls = UNKNOWN_TOOL before pre-execute) and capability_call nested sub-calls must pass; guidance via call return + persona. Phase 3 keeps `mode: both` (native verification + run_code mechanical); kix red line "verification/observation stay native" unchanged. Config: `enableRestrict:false` / `extraResidentTools`. **v1.2.13 (2026-08-17)**: lite `ACTIVATABLE_TOOLS` toolFilter made platform-conditional (fixes v1.2.12 dual-source miss, proven by deploy E2E); orchestration members (qa/dev/reviewer) get `current_sprint: N` injected on capability_call dispatch (workspace marker driven, handoff-gate mechanical backstop, see §5 #8) |
...
6. ~~**kix-guards v3 新增门禁的真实拦截**~~ → **E2E 坐实（2026-08-17，WSL2 dsh web @33236 最新修复版）**：原 2026-08-15 挂账（128 组 → 142 组单元回归全过，但 standing generation 为 v1 时代组装）。最新部署（v9/v1.2.11）新会话实测：**push origin main 拦截**（禁推 main/master）、**push --force origin main 拦截**（force 重写历史）、**控制平面写拦截**（~/.dsh/.agent-presets/ 新建文件也算编辑）、**run_code 受限能力拦截**（child_process 静态扫描）、**gh repo delete 拦截**（破坏性删除）；正向对照 3 项全放行（run_code 纯算术 / 只读 git / 只读 gh）——**5 拦 3 放，0 误报，无绕过通道**。另实测 kix-discipline 上下文注入（编辑后补验证提醒）与模型无绕过行为。**v10（2026-08-17，本轮源码迭代）**：修复 commit-on-main 检查的仓库根解析边界——`cd <repo> && git commit`（无 -C）+ 会话 cwd≠仓库根时 resolveRepoRoot 解析失败致检查静默跳过；新增命令位 cd 提取（`(?:^|[;&|]\s*)cd`，`echo cd /tmp` 不误匹配），非仓库目录 gitRead 失败自然 fail-safe。单元回归扩至 **213 组全过**（含新增 3 组 cd 用例）。**v10.1（2026-08-17，部署复验第二轮实锤）**：v10 只修了 resolveRepoRoot，**整链仍然不拦纯 commit**——`DANGEROUS_GIT` 自开源起就缺 `commit`（只有 commit-tree），`isGitWrite()` 对 `git commit` 返回 false → 2b 门禁整体跳过 → checkGitCommit（分支/预算检查）对普通 commit 永不执行 → **main 分支直接 commit 静默放行**（v1.2.12 的 E2E 声称未覆盖此路径；单测只测 repoRootFromText 未测整链）。修复 = 补 `commit` 进 DANGEROUS_GIT + **真实 git 仓库整链回归 6 组**（cwd=仓库根 main deny / cd 进入 main deny / -C 显式仓库 deny / feature 分支 allow / 非仓库 cwd fail-safe allow / message 含 push·main 不误拦），单元回归 **219 组全过**。部署新会话复验：`cd /root/kix-guards-e2e && git commit`（main）→ **BLAST RADIUS deny「禁止在 main 分支直接 commit」**，模型不绕过（提问用户选择合法路径）——与旧版放行形成对照（23d0f02 曾成功提交）。

7. ~~**lite 档 Linux 环境配置缺陷**（2026-08-17 前轮挂账：subagent_lite 在本 Linux 部署不可用）~~ → **已修复并 E2E 坐实（v1.2.13）**：根因是**双源未同步**——agent.cordis.yml 的 tool-subagent-lite 行与 kix-focus.js `ACTIVATABLE_TOOLS.subagent_lite.config` 快照各持一份 `toolFilter.allow`，v1.2.12 只把 preset 行平台条件化（`!!js process.platform === 'win32' ? 'pwsh' : 'bash'`），**快照仍硬编码 pwsh** → 模型走 `kix_capability_call` 首次使用自动激活（capability_call 挂载用的是 ACTIVATABLE_TOOLS 快照）→ tools.restrict() 报 unknown global tool "pwsh"（部署 E2E 复验①首轮实锤）。修复 = 快照同步平台条件化（`process.platform === 'win32' ? 'pwsh' : 'bash'`，与 preset 行同源）。部署新会话复验：`subagent_lite` 经 capability_call 自动激活 + kix_tool_activate + 直呼均正常（bash 生效，无 unknown-tool 报错），机械核对结果与主线程物证一致。kix-focus 单元回归 **83 → 102 组**。

8. ~~**交接 gate 依赖模型自觉带 `current_sprint: N`**（2026-08-17 前轮挂账：Tri-Block 分派常漏契约行 → 门禁静默放行）~~ → **机械层兜底已落地并 E2E 坐实（v1.2.13）**，双保险：
   - **kix-focus 注入**（capability_call 路径）：编曲成员（qa/dev/reviewer）经 `kix_capability_call` 分派时，工作区存在 `docs/.kixpower-current-sprint` 即自动把契约行注入/修正为 marker 值（纯函数 `injectSprintContractLine`/`readActiveSprint`，返回值带 `sprintInjected` 供模型感知）。部署 E2E 复验：模型按指令**故意省略契约行与 Sprint 字样** → capability_call 返回 `sprintInjected: 1` → kix-orchestration 门禁按注入行触发（`docs/sprint-1/` 缺 plan/progress 的 remind notice）——机械兜底闭环实证。
   - **kix-orchestration v10.1 容错**（直呼路径兜底）：`extractHandoffMeta` 无契约行时从 Tri-Block `[CONTEXT]` 段容错解析 `Sprint N`（范围收窄防误触发，契约行永远优先；激活后模型可直呼 subagent_qa 不经 capability_call，故直呼路径也需兜底）。单元回归：kix-focus **102 组**、kix-orchestration **69 → 77 组** 全绿。
