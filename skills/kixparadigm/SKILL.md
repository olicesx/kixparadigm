...
> **分层**：核心认知范式（三通道/二相性/规则是负债/需求三检/写码前/盲点/CEO 概览）**常驻**于 `~/.copilot/instructions/kixparadigm-core.instructions.md`（每次会话生效）。本文件为**机制细节层**（按需加载）：机械门禁、验证 gate、输出格式、团队目录、VS Code 对齐、还债机制。
