...
| P1 | 9：跨厂商矩阵进 settings + 能力元数据校验 | ✅ 已启用（2026-08-15 前已配置）+ 文档化 | settings.yaml 已有 `zai-coding-cn`（GLM-5.2 路由）+ `zai-vision`（GLM-4.6V 完整 profile）；能力元数据校验 = `resolveModelInfo`（contextWindow/maxTokens/reasoning 档位），见 §3.1 |
