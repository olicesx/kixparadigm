...
3. **exit code 0 即成功**：`pwsh` sync 模式下，**exit code 0 + 无 stderr error = 已成功**。PowerShell 长命令回显被截断（残留如 `d" -Raw -Encoding utf8`）**不是失败信号**，禁止据此重试
