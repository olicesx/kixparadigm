...
- ✅ **blast-radius-check.ps1** → `plugins/kix-guards.js`（pre-execute，5 大门禁/232 断言；v7：reflog %gs 口径 / 过期 sprint 指针回退 / 预算兜底；v8：终端 SQL 命令位判定 / 控制平面只拦写意图 / GitHub 前缀可配置）
...
- **2026-08-15 P0 修复（实证驱动）**：三通道实测发现 kix-guards 全部文本门禁
  ...
  回归测试：`plugins/kix-guards.test.js`（232 组断言，node 直接运行）。
