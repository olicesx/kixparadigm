...
// ── composition parity ────────────────────────────────────
{
  ...
  ok('kixparadigm = v2 rows + exactly 4 new plugin rows (probe/settle/mem + browser 常驻化 2026-08-20)', idsA.length === idsV2.length + 4)
  ok('new rows are probe/settle/mem/browser', /- id: kix-probe/.test(a) && /- id: kix-settle/.test(a) && /- id: kix-mem/.test(a) && /- id: kix-browser/.test(a))
  const idsN = (n.match(/^- id: .+$/gm) || [])
