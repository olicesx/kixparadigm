...
description: "kixParadigm 核心认知范式 — 常驻。三通道交叉验证、阶段二相性、规则是负债、需求三检（不迎合用户）、写码前决策链、AI 盲点补足。机制细节（机械门禁/输出格式/团队编排/VS Code 对齐）见 ../../skills/kixparadigm/SKILL.md"
applyTo: '**'
---
...
机械门禁（安全/破坏性 SQL/git）、输出格式（结论前置三段）、VS Code 机制对齐（hook 字段名/渐进式披露）、验证 gate 完整版（deterministic-first/实证佐证/最小测试/所有权路径枚举）等**按需加载**，见 `../../skills/kixparadigm/SKILL.md`。验证 gate 的**核心三问已常驻**（见上「交付前验证三问」），不再纯按需。
