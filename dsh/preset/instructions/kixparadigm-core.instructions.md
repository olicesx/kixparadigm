...
> 只保留压缩锚点（每次会话生效，v1.2.10 实测约 3.1K token，预算 ≤4K）；本文件是同一认知层的**完整展开**，
...
- **2026-08-16 插件化改造本身就是规则是负债的应用**：纪律从 prompt 说教迁到插件机制，persona 从 7.3K 字符压到 3.3K；v1.2.9 编曲模型落地后回到 5.4K 字符/约 3.1K token（仍在 ≤4K token 预算内，v1.2.10 起由一致性检查守护）
