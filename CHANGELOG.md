...
- **伤重复操作，不回仪式**：`pushTargetsProtectedRef` / `isForcePush` / `isGhDestructive` / `reviewShellMutation` 都按本条调用参数判定——不再把同行 `gh pr create --base main` 当成受保护分支 push，不再把后段 `rm -f` 当成 force-push，不再把 `grep`/`commit -m`/`node -e` 里的 `gh repo delete` 或 `writeFileSync` 字面量当成真删除/真写入。review epoch 对 `git branch`/`git config`/`stash`/`remote`/`tag`/`notes`/`worktree`/`reflog` 按参数分读写——`git branch -a` / `git stash list` / `git remote -v` 放行，`-D` / `stash drop` / `tag v1` 仍拦。`kix_discipline_spec` 落盘失败返回 `ok: false`（ctx.fs 失败回退 node:fs）；`kix-signal` 看见磁盘上完整 `spec.md` 不再催再调一遍。
