...
function installPreset(log) {
  const src = path.join(PKG_ROOT, 'dsh', 'preset')
...
function installVisionBridge(log) {
  ...
  const source = path.join(profile, 'plugins', BRIDGE_NAME)
  const junction = path.join(profile, 'node_modules', BRIDGE_NAME)

  if (!fs.existsSync(path.join(PKG_ROOT, 'dsh', 'vision-bridge'))) {
    log.warn('本包不含 vision-bridge 源码，跳过')
    return
  }
  log.step(`安装 vision-bridge → ${source}`)
  copyTree(path.join(PKG_ROOT, 'dsh', 'vision-bridge'), source, log)
  ...
  try {
    switch (cmd) {
      ...
      case '--version': case '-v': case 'version':
        console.log(require(path.join(PKG_ROOT, 'package.json')).version); break
      case '--help': case '-h': case 'help':
        console.log(`kixparadigm — kix 范式全家桶一键导入
用法:
